    ...
    const PartitionIdToMaxBlock * max_block_numbers_to_read) const
{
    ...
    if (use_sampling)
    {
        for (auto & pipe : res)
        {
            ...
            pipe.emplace_back(std::make_shared<FilterTransform>(output.getHeader(), filter_expression, filter_function->getColumnName()));
    ...
    const Settings & settings) const
{
    ...
    if (sum_marks > 0 && settings.merge_tree_uniform_read_distribution == 1)
    {
        ...
        {
            res.emplace_back(std::make_shared<MergeTreeThreadSelectBlockInputProcessor>(
